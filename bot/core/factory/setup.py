...
def setup_filters(dispatcher: Dispatcher) -> None:
    ...
    """
    dispatcher.update.filter(F.chat.type.in_({ChatType.PRIVATE, ChatType.GROUP}))
...
def setup_outer_middlewares(dispatcher: Dispatcher, config: Config) -> None:
    ...
    """
    pool = dispatcher["session_pool"] = create_pool(dsn=config.postgres.dsn)
    i18n_middleware = dispatcher["i18n_middleware"] = I18nMiddleware(
        core=FluentRuntimeCore(
            path="locales/{locale}",
            raise_key_error=False,
            locales_map={Locale.UK: Locale.EN, Locale.EN: Locale.UK},
        ),
        manager=UserManager(),
